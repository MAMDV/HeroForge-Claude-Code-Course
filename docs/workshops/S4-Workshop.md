...
pwd               # Should show your LifeOps project path
ls src/           # Should show your current source files
```
...
```bash
git checkout -b yourname-workshop-4
```

**Or just tell Claude:**

```bash
claude "Create and switch to a new Git branch called yourname-workshop-4"
```

You should now be on the `yourname-workshop-4` branch. Confirm with:

```bash
git branch --show-current   # Should show yourname-workshop-4
```
...
In your terminal, use Claude Code to create CLAUDE.md:

```bash
claude "Create a CLAUDE.md file in the project root for the LifeOps Command Center app. Include:
...
Use Claude Code to set up the React project:

```bash
claude "Convert this project to use React with Vite. Set up Tailwind CSS for styling. Keep the existing package.json name and description. Create the standard Vite + React structure with:
...
```bash
claude "Create src/context/AppContext.jsx with a React context and useReducer. The state should have:
...
```bash
claude "Create src/hooks/useLocalStorage.js — a custom React hook that works like useState but persists to localStorage. It should:
...
```bash
claude "Create these React components for the LifeOps Command Center. Each should be in src/components/ and use Tailwind CSS for styling:
...
```bash
claude "Add a 'priority' filter to the TaskList component. Users should be able to filter tasks by priority (all, high, medium, low). This requires:
...
```bash
claude "Add a 'Component Conventions' section to CLAUDE.md that documents:
...
```bash
claude "Create a file .claude/commands/plan.md with this structure:
...
```bash
claude "Create a file .claude/commands/review.md with this structure:
...
```bash
claude "Create .claude/commands/handoff.md that:
...
```bash
claude "Create .claude/commands/pickup.md that:
